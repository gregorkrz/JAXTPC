...
def _plot_global_fit(trans_fit, long_fit, out_pdf: Path):
    ...
    for ax, fit, title in [
        ...
        order = np.argsort(x)
        ax.scatter(x, y, s=8, alpha=0.45, label='samples')
        ax.plot(x[order], y_fit[order], lw=2.0, color='tab:red', label='linear fit')
...
def main():
    ...
    for i, sample in enumerate(segment_samples, start=1):
        ...
        arr = np.asarray(arrays[vol_idx * n_planes + plane_idx], dtype=np.float64)

        abs_arr = np.abs(arr)
        wire_prof = np.sum(abs_arr, axis=1)
        time_prof = np.sum(abs_arr, axis=0)

        _, var_wire_idx = _weighted_mean_variance(wire_prof)
        _, var_time_idx = _weighted_mean_variance(time_prof)
        if not np.isfinite(var_wire_idx) or not np.isfinite(var_time_idx):
            continue
        ...
        sigma_wire_cm = float(np.sqrt(var_wire_idx) * wire_spacing_cm)
        sigma_time_us = float(np.sqrt(var_time_idx) * time_step_us)
        sigma_long_cm = float(sigma_time_us * velocity_cm_us)
        ...
        measurements.append({
            ...
            'sigma_wire_cm': sigma_wire_cm,
            'sigma_long_cm': sigma_long_cm,
            'sigma_time_us': sigma_time_us,
            'wire_variance_idx2': float(var_wire_idx),
            'time_variance_idx2': float(var_time_idx),
    ...
    sigma_trans = np.array([m['sigma_wire_cm'] for m in measurements], dtype=np.float64)
    sigma_long = np.array([m['sigma_long_cm'] for m in measurements], dtype=np.float64)
    ...
    for tname in track_names:
        ...
        ts = np.array([m['sigma_wire_cm'] for m in tm], dtype=np.float64)
        ls = np.array([m['sigma_long_cm'] for m in tm], dtype=np.float64)
    ...
    print(f"Global D_T = {trans_fit['D_cm2_per_us']:.6e} ± {trans_fit['D_err_cm2_per_us']:.2e} cm^2/us"
          f"  (R^2={trans_fit['r2']:.4f})")
    print(f"Global D_L = {long_fit['D_cm2_per_us']:.6e} ± {long_fit['D_err_cm2_per_us']:.2e} cm^2/us"
